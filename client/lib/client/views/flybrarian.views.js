...
export const views = {
            ...
            this.model.lineup.forEach(function (event) {
                ...
                if (event.who) {
                    ...
                    if (event.who && Array.isArray(event.who)) {
                        ...
                        artistNode.innerText = event.who.name + '  ';
                    }
    ...
    Timeline: function (model) {
        return View(model, function () {
            ...
            const start = new Date(model.start), end = new Date(model.end);
            const startDay = start.getDay(), startMin = start.getMinutes(), endDay = end.getDay(), endMin = end.getMinutes();
            ...
            for (var i = 0; i < hours; i++) {
                const slot = create('div', 'timeslot'), slot30 = create('div', 'timeslot'), slotLast = create('div', 'timeslot');
