...
const performancesJSON = performances;
const performersJSON = performers;
const genres = categories.music.genres, evGap = utils.createGap, sortEvents = utils.sortEvents;
...
function idGenerator(identifier) {
    if (!identifier)
        return identifier;
...
performancesJSON.performances.forEach(function (performance) {
    ...
    viewModel.where.events = viewModel.where.events || new Set();
...
export const eventService = {
    ...
    lineups: {
        // options:
        // - filter
        byDay: function (day, stages, options) {
            ...
            const startHour = state.start.getHours(), endHour = (state.end) ? state.end.getHours() : 0;
