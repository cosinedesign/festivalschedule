...
export const dispatcher = (function () {
    ...
    return {
        ...
        init: async function (appState) {
            ...
            if (!state.current)
                return dispatcher.moveTo('home');
        },
        moveTo: async function moveTo(c) {
            if (!c)
                return; // <-- HEY TYPESCRIPT, THIS IS A THING THAT KEEPS 2322 AT BAY, DUMMY
            if (typeof c === 'string') {
                return dispatcher.moveTo(dispatcher.contexts.get(c));
            }
            if (state.current)
                state.current.unload(state.appState);
